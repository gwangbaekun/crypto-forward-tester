...
def run() -> None:
    ...
    def on_message(c, message):
        ...
        if isinstance(payload, ProtoOAErrorRes):
            ...
            if sym:
                print("\n  심볼 스펙:")
                print(f"  symbolId      : {sym.symbolId}")
                print(f"  lotSize       : {sym.lotSize}")
                print(f"  minVolume     : {sym.minVolume}")
                print(f"  maxVolume     : {sym.maxVolume}")
                print(f"  stepVolume    : {sym.stepVolume}")
                print(f"{'━'*54}")
