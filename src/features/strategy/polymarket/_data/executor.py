...
Polymarket V2 최소 주문 규칙:
  - shares ≥ 5          (maker/post-only 거절 방지)
  - shares × price ≥ $1 (marketable BUY 거절 방지)
  두 조건을 동시에 만족하는 최솟값을 자동 계산한다.
"""
...
def _min_order(price: float) -> tuple[float, float]:
    ...
    - shares ≥ 5
    - shares × price ≥ $1.02  (2% 마진으로 반올림 거절 방지)
    shares 는 소수 2자리 올림 처리.
    """
    if price <= 0:
        raise ValueError(f"price 가 0 이하: {price}")
    min_for_dollar = _MIN_USD_MARGIN / price
    shares_raw = max(_MIN_SHARES, min_for_dollar)
