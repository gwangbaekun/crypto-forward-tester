...
로드맵 §5. **레짐 스위치가 아니라 레벨 전략이다** (T2 발견 3):
콜+/풋− 규약에서 SPY 의 GEX 부호는 시간에 따라 바뀌는 레짐이 아니라 거의 항상
음수인 구조적 상수였다. 플립 위/아래로 모드를 바꾸는 설계는 전환할 레짐이 없다.
남는 것은 **딜러 헤지가 몰리는 행사가(벽)** 이고, 그건 부호와 무관하게 존재한다.

메커니즘 한 문장 (§0 의 채택 기준):
    딜러는 옵션 재고를 델타 헤지해야 하고, SPY 에서 그 규모는 1% 이동당 $10bn —
    하루 거래대금의 26% 다. 그 헤지 주문이 몰리는 행사가가 지지·저항으로 작동한다.
    (BTC 는 같은 값이 0.16% 라 이 논리가 성립하지 않는다 — T2 발견 1)

**이건 알림봇이 아니라 원장이다.** n=9 로는 아무것도 판정할 수 없고
(|ρ|=0.131 검출에 459 세션 필요), 그래서 이 모듈의 목적은 신호를 쏘는 게 아니라
