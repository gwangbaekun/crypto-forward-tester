"""
ETH CVD Explosion — Signal Function (forwardtest 호환)
...
진입 시점:
  15m 봉 종가 (현재봉 기준) — look-ahead bias 없음
...
def compute_signal(
    ...
    *,
    ...
    """
    ETH CVD Explosion 신호 계산.

    Parameters
    ----------
    current_price    : float — 현재 가격 (15m 종가)
    sweep_by_tf      : {"15m": {"data": [...]}, "4h": {"data": [...]}}
...
def _position_meta_for_entry(tpsl_params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if mode in (MODE_MAGNET_RR, MODE_MAGNET_TP_RR):
        ...
        meta["sl_lift_mode"]              = str(tpsl_params.get("sl_lift_mode") or "always")
        meta["sl_lift_min_intensity"]     = str(tpsl_params.get("sl_lift_min_intensity") or "HIGH").upper()
        meta["sl_lift_rank_le"]           = int(tpsl_params.get("sl_lift_rank_le") or 2)
